...
export default async function IncomingsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <section>
        <h2 className="mb-2 text-sm font-medium text-gray-700">Журнал поступлений</h2>
        {incomings.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-100">
                {incomings.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-500">{i.receivedAt.toLocaleDateString("ru-RU")}</td>
                    <td className="px-4 py-3 text-gray-700">{i.project?.client?.name ? `${i.project.client.name} · ` : ""}{i.project?.name ?? "—"}</td>
                    <td className="px-4 py-3 text-right font-medium text-gray-900">{formatTiyn(i.amount)}</td>
